...
public class PolygonActivity extends Activity implements
	...
	private void setUpMap() {
		...
		lMap.moveCamera(CameraUpdateFactory.newLatLngZoom(Constants.BEIJING, 5));// 设置指定的可视区域地图
		// 绘制一个长方形
		lMap.addPolygon(new PolygonOptions()
		...
		for (int i = 0; i <= numPoints; i++) {
			options.add(new LatLng(Constants.WUHAN.latitude
					+ semiVerticalAxis * Math.sin(i * phase),
					Constants.WUHAN.longitude + semiHorizontalAxis
							* Math.cos(i * phase)));
		}
		// 绘制一个椭圆
		polygon = lMap.addPolygon(options.strokeWidth(25)
